...
\begin{document}
\title{Unsupervised Conflict Index Construction and Risk Forecasting for Social Media Comment Streams}
...
\subsection{Factors Governing Conflict-Index Forecastability}\label{sec:discussion}\label{sec:attribution}
...
Experimental results under a strict temporal-split protocol demonstrate the effectiveness of the proposed framework. On synthetic conflict trajectories, deep sequential forecasters achieve strong performance (BiGRU R\textsuperscript{2}=$0.811\pm0.009$, CNN-BiLSTM R\textsuperscript{2}=$0.809\pm0.010$), with Transformer close behind ($0.806\pm0.007$), all significantly outperforming non-neural baselines. On real Weibo data with dense commenting activity, comment volume is robustly predictable (R\textsuperscript{2}=$0.715$), confirming that the forecasting architecture generalizes to real-world time series. Component ablation verifies that each conflict-index component contributes complementary information, and weight sensitivity analysis confirms robustness to moderate parameter variations. We further identify that the temporal autocorrelation of the target signal is the key factor governing forecastability: signals with strong autocorrelation (synthetic trajectories, comment volume) are highly predictable, while text-derived conflict signals at coarse granularity exhibit low autocorrelation and limited predictability---a finding that clarifies the boundary conditions for practical deployment. These results establish the proposed unsupervised conflict index as a valid cross-sectional monitoring tool and the CNN-BiLSTM forecasting pipeline as an effective temporal predictor, while also highlighting directions for future improvement in signal quality and temporal granularity.
