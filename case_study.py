...
Uses REAL pretrained BERT sentiment model (nlptown/bert-base-multilingual-uncased-sentiment)
...
# ═══ Real Pretrained Model Conflict Computer ═══
class RealConflictComputer:
    """Uses real HuggingFace sentiment model + sentence transformer for stance."""
    def __init__(self):
        ...
        self.emb_model = SentenceTransformer(
            "paraphrase-multilingual-MiniLM-L12-v2", device=DEV)
        print("  Models loaded.")
    ...
    def compute_batch(self, texts):
        ...
        # Attack: 1-star + half of 2-star
        attack = probs[:, 0] + 0.5 * probs[:, 1]
        # Emotion: high-arousal negative (1-star = anger, 1+2-star = negative)
...
def train_model(Xtr, ytr, Xte, yte, device=DEV):
    """Train CNN-BiLSTM and return predictions + metrics."""
    tl = DataLoader(TensorDataset(Xtr, ytr), 64, True)
    vl = DataLoader(TensorDataset(Xte, yte), 128)
    model = CNNBiLSTM().to(device); opt = torch.optim.Adam(model.parameters(), lr=1e-3)
    ...
    with torch.no_grad(): return model(Xte.to(device)).cpu().numpy()

def metrics(y_pred, y_true, esc_labels):
    """Compute MAE, R², Escalation F1."""
    mae = float(np.mean(np.abs(y_pred - y_true)))
    ss_r = np.sum((y_true - y_pred)**2); ss_t = np.sum((y_true - y_true.mean())**2)
    r2 = 1 - ss_r / (ss_t + 1e-8)
    pe = (y_pred.max(1) >= 0.65).astype(int); te = esc_labels
    tp, fp, fn = (pe & te).sum(), (pe & (1 - te)).sum(), ((1 - pe) & te).sum()
    p = tp / (tp + fp) if (tp + fp) else 0; r = tp / (tp + fn) if (tp + fn) else 0
    return {"mae": mae, "r2": r2, "esc_f1": 2 * p * r / (p + r) if (p + r) else 0}
...
if __name__ == "__main__":
    ...
    print(f"Selected {len(selected)} events for case study")
    ...
    print(f"Total: {len(df_all)} comments across {len(selected)} events")

    # ── Compute conflict index with REAL pretrained model ──
    print("\n" + "=" * 60)
    ...
    # Fusion: c = sigmoid(w_a*a + w_e*e + w_s*s)
    df_all["c"] = (1.0 / (1.0 + np.exp(-(0.5 * a_cal + 0.3 * e_cal + 0.2 * s_cal)))).clip(0, 1)
    df_all["bin"] = df_all["dt"].dt.floor("2h")

    print(f"  Attack μ={r['attack'].mean():.3f} σ={r['attack'].std():.3f}")
    print(f"  Emotion μ={r['emotion'].mean():.3f} σ={r['emotion'].std():.3f}")
    print(f"  Stance  μ={s_raw.mean():.3f} σ={s_raw.std():.3f}")
    print(f"  Conflict index μ={df_all['c'].mean():.3f} σ={df_all['c'].std():.3f}")
    ...
    for evt_name in df_all["event_name"].unique():
        ...
        # Get reversal window
        fb = edf["fbegin"].iloc[0]; fe = edf["fend"].iloc[0]
        ...
        if len(agg) < L + H + 10:
            print(f"  {evt_name}: insufficient bins ({len(agg)}), skipping")
            continue
        ...
        if len(Xs) < 30:
            print(f"  {evt_name}: insufficient windows ({len(Xs)}), skipping")
            continue
        ...
        # Train CNN-BiLSTM
        print(f"\n  Training CNN-BiLSTM on {evt_name} ({n_tr} train, {n - n_tr} test)...")
        y_pred = train_model(Xtr, ytr, Xte, yte)

        # Metrics
        m_cnn = metrics(y_pred, yte_np, esc_te)

        # Baselines
        Xte_f = Xte[:, :, 0].cpu().numpy()
        yp_persist = np.tile(Xte_f[:, -1].reshape(-1, 1), (1, H))
        Xar_te = Xte[:, -6:, 0].cpu().numpy()
        Xar_tr = Xtr[:, -6:, 0].cpu().numpy()
        ...
        m_persist = metrics(yp_persist, yte_np, esc_te)
        m_ar = metrics(y_ar, yte_np, esc_te)
        m_xgb = metrics(y_xgb, yte_np, esc_te)
        ...
        event_res = {
            ...
            "CNN-BiLSTM": m_cnn,
            "Persistence": m_persist, "AR(6)": m_ar, "XGBoost": m_xgb,
            "autocorr": np.corrcoef(vals[:-1], vals[1:])[0, 1],
            "pre_rev_c": pre_rev_c, "during_rev_c": during_rev_c,
            "trajectory": vals_raw, "reversal_mask": (agg["rev_pct"] > 0).values,
        }
        results_per_event[evt_name] = event_res

        print(f"    CNN-BiLSTM: R²={m_cnn['r2']:.4f}  Esc-F1={m_cnn['esc_f1']:.4f}  "
              f"Δrev={during_rev_c - pre_rev_c:+.4f}")
        print(f"    vs Persistence: R²={m_persist['r2']:.4f}  XGBoost: R²={m_xgb['r2']:.4f}")
    ...
    # ── Figure ──
    best_name = sorted(results_per_event.keys(),
                       key=lambda k: results_per_event[k]["CNN-BiLSTM"]["r2"], reverse=True)[0]
    ...
    print(f"\nDone in {tmin:.1f} min")
    notify(f"Case study (BERT) done! {len(results_per_event)} events in {tmin:.1f}min")
