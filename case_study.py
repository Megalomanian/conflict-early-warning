...
plt.rcParams.update({"font.family":"serif","font.size":9,"axes.titlesize":10,"figure.dpi":150,"savefig.dpi":300,"savefig.bbox":"tight"})
...
if __name__=="__main__":
    ...
    # Use top 3 events with clear escalation
    results_per_event={}
    all_preds=[]; all_trues=[]
    ...
    for evt_idx, (_, row) in enumerate(meta.head(3).iterrows()):
        ...
        # Compute conflict index per comment
        ...
        for j in range(len(vals)-L-H+1):
            ...
            # Ground truth escalation: does GT reversal happen in this horizon?
    ...
    # ═══ Figure: Case study visualization ═══
    # Pick best event
    ...
    bars=ax.bar(models,r2s,color=cols,width=0.5)
    ...
    # (c) Escalation confusion
    ax=axes[2]
    esc_names=["Persistence","AR(6)","XGBoost","CNN-BiLSTM"]
    esc_f1s=[best["Persistence"]["esc_f1"],best["AR(6)"]["esc_f1"],best["XGBoost"]["esc_f1"],best["CNN-BiLSTM"]["esc_f1"]]
    cols2=["#95a5a6","#95a5a6","#95a5a6","#e74c3c"]
    bars=ax.bar(esc_names,esc_f1s,color=cols2,width=0.5)
    for bar,v in zip(bars,esc_f1s):
        ax.text(bar.get_x()+bar.get_width()/2,bar.get_height()+0.02,f"{v:.3f}",ha="center",fontsize=8)
    ax.set_ylabel("Escalation F1"); ax.set_title("(c) Reversal Detection")
    ax.grid(alpha=0.3,axis="y")

    fig.suptitle("Real-World Case Study: Weibo Public Opinion Reversal Events",fontsize=11,fontweight="bold")
    fig.tight_layout(); fig.savefig(f"{R_DIR}/fig_case_study.pdf"); plt.close(fig)
    print(f"\nSaved {R_DIR}/fig_case_study.pdf")
