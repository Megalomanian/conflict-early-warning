...
Figure~\ref{fig:trajectory} shows a sample synthetic trajectory and a representative 6-step forecast. Figure~\ref{fig:model_comp} presents the full model comparison with error bars. The CNN-BiLSTM model converges stably within approximately 75 epochs under early stopping (Figure~\ref{fig:training}), with the validation loss plateauing after an initial rapid decrease.
...
\caption{CNN-BiLSTM training convergence. Validation loss plateaus within $\sim$75 epochs; early stopping (patience 25) selects the model at the minimum validation loss.}
\label{fig:training}
\end{figure}
...
\subsection{Signal Degradation under Noise}

To assess robustness, we inject Gaussian noise of increasing magnitude ($\sigma \in \{0.03, 0.06, 0.10\}$) into the input trajectories to simulate degraded component estimates. Table~\ref{tab:noise} reports the forecasting degradation. CNN-BiLSTM maintains R\textsuperscript{2}$>0.63$ even under the strongest noise ($\sigma=0.10$), demonstrating reasonable resilience. Escalation F1 degrades more sharply, suggesting that noise in the conflict index primarily affects the model's ability to discriminate escalation from non-escalation windows.

\begin{table}[h]
\caption{Effect of signal degradation on CNN-BiLSTM forecasting performance.}
\label{tab:noise}
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Noise $\sigma$} & \textbf{R\textsuperscript{2}} & \textbf{Esc-F1} \\
\midrule
0.00 (clean) & 0.806 & 0.347 \\
0.03 & 0.757 & 0.360 \\
0.06 & 0.696 & 0.358 \\
0.10 & 0.635 & 0.271 \\
\bottomrule
...
To assess whether forecasting performance is sensitive to the fusion weights $(w_a, w_e, w_s)$, we conduct a random search over 40 configurations sampled from a Dirichlet distribution. The R\textsuperscript{2} across configurations varies within a narrow range (interquartile range $<0.015$), indicating that forecasting performance is robust to moderate weight variations around the default setting $(0.5, 0.3, 0.2)$. The optimal configuration found through random search lies close to the default weights, supporting the reasonableness of the chosen values. We therefore recommend the default weights as a practical starting point, with tuning providing only marginal gains.

\subsection{Real-Data Case Study: Weibo Reversal Events}

We apply the framework to three Weibo public-opinion reversal events---Huolala incident (22K comments), Zhang Zhehan incident (22K comments), and Gou Jing exam fraud (20K comments)---using 2-hour time bins. The conflict index shows modest but consistent positive shifts during reversal windows (mean increase $+0.005$ to $+0.015$). CNN-BiLSTM consistently outperforms Persistence and AR baselines on forecasting R\textsuperscript{2} for the Huolala and Zhang Zhehan events, the Huolala incident achieving Esc-F1 of 0.889 (the strongest signal), Zhang Zhehan at 0.240, and Gou Jing at 0.000 (Figure~\ref{fig:case}). Performance varies by event type: events with clear opinion polarization (Huolala, Zhang Zhehan) show stronger conflict-index modulation than those driven primarily by information revelation (Gou Jing).

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{../figures/fig_case_study.pdf}
\caption{Conflict index trajectories for three Weibo reversal events. Red shaded regions mark the annotated reversal windows. Conflict index rises modestly during reversal (Huolala $\Delta c=+0.011$, Zhang Zhehan $+0.018$, Gou Jing $+0.008$).}
\label{fig:case}
\end{figure}
...
Weight sensitivity analysis (random search over 40 Dirichlet-sampled configurations) confirms that the fusion design is robust to moderate variations around the default weights $(0.5, 0.3, 0.2)$.
...
The real-data case study on Weibo reversal events confirms that the unsupervised conflict index shows mixed results: the Huolala incident produces a clear conflict-index shift (Esc-F1=0.889), but Zhang Zhehan (0.240) and Gou Jing (0.000) show weak or no detectable escalation signal. The modest effect sizes ($+0.005$ to $+0.015$ during reversal windows) highlight the gap between synthetic and real-world performance. This gap is expected: synthetic trajectories have clean escalation dynamics, while real comment streams contain substantial noise from topic drift, platform-specific discourse norms, and heterogeneous user behavior.
