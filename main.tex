...
The conflict index is unsupervised by construction, so its measurement quality must be checked against human judgments. We validate the attack component---the largest-weight element of the fusion---on the Wikipedia Detox personal-attacks corpus~\cite{Wulczyn2017}: 191,634 English comments with binary human labels (10.1\% positive). Using the pipeline's exact scoring path (the multilingual toxicity model, 256-token truncation), the toxicity score separates attack-labeled from unlabeled comments at AUC $0.988$ (Spearman $\rho=0.507$, Pearson $r=0.557$; both $p<0.001$), and the mean score is $0.806$ for labeled versus $0.033$ for unlabeled comments---a separation nearly two orders of magnitude larger than the review-sentiment proxy used in an earlier version of this pipeline (AUC $0.744$, $\rho=0.254$). The attack component is therefore a statistically strong measure of personal attack. The anger component (zero-shot NLI) and the stance component are evaluated structurally (Sec.~\ref{sec:stance_audit}) rather than against external labels; expert annotation of the Chinese-language components remains ongoing work.
