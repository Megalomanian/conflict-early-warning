...
We select the three largest events---\textbf{货拉拉乘客坠车身亡} (Huolala passenger death, 32K comments), \textbf{张哲瀚事件} (Zhang Zhehan incident, 22K comments), and \textbf{苟晶高考被替事件} (Gou Jing exam fraud, 20K comments)---and apply the CNN-BiLSTM forecasting pipeline with 2-hour bins. For this case study, the conflict index is computed using a simplified keyword-based implementation of the three-component formulation described in Section~\ref{sec:conflict_index}; the full pretrained BERT-based pipeline (Section~\ref{sec:impl}) is validated on the Zhihu dataset. Figure~\ref{fig:case_study} visualizes results for the Huolala event.
