...
On held-out Zhihu periods, the selected classifier achieves AUC $0.897$ with a significant recall gain over persistence, and---unlike exact-value regression---generalizes to entirely unseen topics (5-fold topic-held-out F1 $0.787\pm0.020$ vs.\ persistence $0.705$). The pipeline also transfers to 27 Weibo reversal events (AUC $0.989$). Threshold and feature ablations show that causal state history drives the gain, while exact-value regression remains persistence-dominated.
...
Together, these experiments establish a hierarchy of forecastability: objective activity signals are strongly predictable; cleaned causal state histories support useful high-risk-state prediction; and exact raw semantic trajectories remain difficult even in dense commenting environments. The selected classifier relies on state history alone, but this concerns \emph{classifier inputs}: the index-level ablation of Sec.~\ref{sec:component_ablation} shows that the semantic components remain essential through the index construction itself.
...
\midrule
70th & 0.456 & 0.899 & 0.784 & 0.802 & 0.799 \\
80th & 0.421 & 0.903 & 0.822 & 0.799 & 0.758 \\
90th & 0.307 & 0.913 & 0.770 & 0.770 & 0.734 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Cross-Platform Generalization: Weibo Reversal Events (RQ2)}\label{sec:weibo_transfer}
...
To assess sensitivity to fusion weights $(w_a, w_e, w_s)$, we re-run the risk-state pipeline for 40 configurations sampled from a Dirichlet distribution (fixed span 48, full feature pool). Test F1 ranges over $[0.712, 0.828]$ (mean $0.784$, std $0.027$), and the best configuration is attack-dominant $(w_a,w_e,w_s) = (0.26, 0.00, 0.74)$ with F1 $0.828$ and AUC $0.919$. The default $(0.5, 0.3, 0.2)$ lies near the middle of this range. Consistent with the component ablation, configurations that down-weight the attack component lose F1, while the emotion weight contributes little; the stance weight can compensate for attack in the fusion, but not when attack is removed entirely.
...
The analysis points to (c) as the dominant difficulty, amplified by (a). The positive classifier succeeds by changing the operational question from exact score extrapolation to whether a persistent, topic-relative high-risk state will occur. Validation selects state history alone: adding activity, missingness, or raw semantic components lowers validation F1. This target is appropriate for monitoring, but it should not be confused with precise prediction of every raw conflict-score fluctuation.
