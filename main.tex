...
Table~\ref{tab:ew_results} evaluates the trigger rules proposed in Section~\ref{sec:warning_rules} on CNN-BiLSTM forecasts under the temporal-split protocol. The \textbf{Threshold + Growth} combination achieves the best overall F1 (0.43), while all rules maintain perfect precision---when the model issues an alert, escalation is indeed occurring. However, recall is low across all rules (0.07--0.27), confirming that under strict temporal ordering, the model rarely anticipates escalation in advance, consistent with the negative mean lead time reported above. The \textbf{Threshold} rule alone provides the highest recall (0.22) among single-condition rules, while \textbf{Composite} achieves the lowest alert rate (1.8\%), suitable for high-confidence scenarios. These results validate the practical utility of the trigger framework for conservative risk monitoring (RQ5), while highlighting the fundamental challenge of achieving high-recall early detection from short-horizon forecasts. Figure~\ref{fig:ew} illustrates trigger behavior on a sample trajectory.
...
\midrule
Threshold (Rule~1) & 1.00 & 0.22 & 0.37 & 3.9\% \\
Growth (Rule~2) & 1.00 & 0.07 & 0.14 & 1.3\% \\
Thr + Growth & 1.00 & 0.27 & 0.43 & 4.7\% \\
Composite (Rule~5) & 1.00 & 0.11 & 0.19 & 1.8\% \\
\bottomrule
\end{tabular}
\vspace{4pt}
\footnotesize{Trigger evaluation on synthetic test set ($\sim$2,745 windows, strict temporal split) with $\eta{=}0.65$, $\gamma{=}0.1$. All rules achieve perfect precision under temporal ordering---when the model issues an alert, escalation is indeed occurring---but recall is low (0.07--0.27), consistent with the negative mean lead time ($-1.7$ bins). The Thr+Growth combination achieves the best F1 (0.43).}
\end{table}
