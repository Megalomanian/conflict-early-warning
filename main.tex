...
\begin{table}[t]
\caption{Forecasting results on synthetic trajectories (temporal split: 75\% train / 25\% test, $L{=}12$, $H{=}6$). Metrics are mean $\pm$ std over 5 independent seeds. All results under strict temporal ordering with ECDF calibration fitted on training data only.}\label{tab:forecast_results}
\centering
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[t]
\caption{Real-data forecasting results under temporal split. Conflict index: Zhihu topics at 12h bins; Comment volume: Weibo events at 1h bins. All results under strict temporal ordering with train-only normalization. Weibo data: 6,407 windows from 27 events. Zhihu data: 2,373 windows from 20 topics.}\label{tab:real_results}
\centering
\begin{tabular}{lcc}
\toprule
