...
\begin{abstract}
Social media conflict forecasting promises earlier intervention than post-hoc content classification, yet raw text-derived scores are sparse and temporally noisy. We construct a conflict index without task-specific labels and introduce a leakage-resistant cleaning pipeline that regularizes the 12-hour time axis, learns all calibration and cleaning statistics from training data only, and performs causal exponential smoothing. A logistic model predicts whether the stabilized state will exceed the topic-specific training-period 80th percentile within the next six bins. Smoothing, regularization, decision threshold, and feature-set complexity are selected on validation data; the final 25\% of each topic is held out for testing. The validation rule selects a parsimonious state-history model. Across 20 Zhihu topics and 1,706 test windows, it achieves AUC $0.891$, precision $0.738$, recall $0.822$, and F1 $0.778$, compared with persistence recall $0.584$ and F1 $0.716$. Topic-level paired bootstrap confirms gains in F1 ($+0.062$, 95\% CI $[0.018,0.108]$) and recall ($+0.241$, $[0.173,0.317]$); F1 improves on 15 of 20 topics. Results remain positive at 70th, 80th, and 90th-percentile risk definitions. Continuous-value regression remains below persistence, showing that causal state modeling supports high-risk-state prediction but not exact trajectory forecasting. Leave-one-out ablation at the index level shows that every semantic component contributes to the risk-state target, with stance polarization the most influential. The attack component is validated against 191,634 human Wikipedia Detox labels (AUC $0.744$, Spearman $\rho=0.254$); the same pipeline transfers to 27 Weibo reversal events (AUC $0.936$); and a reproduction of pooled random splits quantifies R\textsuperscript{2} inflation from protocol leakage on real data.
\end{abstract}
...
We operationalize a candidate \emph{conflict index} by combining attack-related negativity, negative high-arousal emotion, and stance polarization into a unified post-level score without task-specific conflict labels. This unsupervised construction stage is distinct from the downstream forecasters, which are trained with historical trajectory targets. Its attack component is validated against 191,634 human-annotated English Wikipedia Detox comments (Spearman $\rho=0.254$, AUC $0.744$), and leave-one-out ablation at the index level shows that every semantic component contributes to the downstream risk-state target, with stance polarization the most influential.
...
\textbf{Method} & \textbf{AUC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
Persistence & \textbf{0.955} & \textbf{0.941} & 0.821 & \textbf{0.877} \\
Logistic (state history) & 0.936 & 0.647 & \textbf{0.957} & 0.772 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item $N=275$ test windows from 20 events. Paired event-bootstrap 95\% CIs for the logistic-minus-persistence difference: recall $+0.174$ $[0.027,0.500]$; F1 $-0.096$ $[-0.217,0.001]$; AUC $-0.021$ $[-0.048,-0.003]$.
\end{tablenotes}
\end{threeparttable}
\end{table}

The pipeline transfers cleanly: absolute test AUC reaches $0.936$, comparable to Zhihu ($0.891$), confirming that the method is not overfitted to a single platform. On acute reversal events, however, the persistence baseline is exceptionally strong (AUC $0.955$, F1 $0.877$): the smoothed risk state is so autocorrelated that carrying the last observation forward already performs well. The logistic model still lifts recall substantially ($+0.174$; event-bootstrap CI excludes zero), at the cost of precision. We therefore read the Weibo result as evidence of transferability with an honest boundary: the relative advantage over persistence that holds on Zhihu does not replicate when the target series is nearly persistent.

\subsection{Construct Validity of the Attack Component}\label{sec:construct_validity}

The conflict index is unsupervised by construction, so its measurement quality must be checked against human judgments. We validate the attack component---the largest-weight element of the fusion---on the Wikipedia Detox personal-attacks corpus~\cite{Wulczyn2017}: 191,634 English comments with binary human labels (10.1\% positive). Using the exact scoring path of the pipeline ($p(\text{1-star}) + 0.5\,p(\text{2-star})$ from the multilingual review-sentiment model, 256-token truncation), the attack score separates attack-labeled from unlabeled comments at AUC $0.744$ (Spearman $\rho=0.254$, Pearson $r=0.271$; both $p<0.001$), and the mean attack score is $0.638$ for labeled versus $0.417$ for unlabeled comments. A social-media-domain sentiment model (XLM-T)~\cite{barbieri2022xlmt} reaches AUC $0.811$ and Spearman $\rho=0.324$ on the same labels, indicating that the review-domain model leaves measurable headroom. The attack proxy is therefore statistically valid but only moderately strong; the emotion and stance components, and Chinese-language discourse, still require expert annotation, which we leave as ongoing work.
...
\midrule
Full & 0.50, 0.30, 0.20 & \textbf{0.778} & \textbf{0.891} & \textbf{0.764} & \textbf{0.875} \\
Drop attack (A) & 0.00, 0.60, 0.40 & 0.782 & 0.839 & 0.713 & 0.865 \\
Drop emotion (E) & 0.71, 0.00, 0.29 & 0.737 & 0.889 & 0.721 & 0.854 \\
Drop stance (S) & 0.63, 0.38, 0.00 & 0.684 & 0.864 & 0.713 & 0.843 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item Fixed-span deltas vs.\ Full: $\Delta$F1 $-$0.051 (A), $-$0.043 (E), $-$0.051 (S); $\Delta$AUC $-$0.010 (A), $-$0.021 (E), $-$0.032 (S). The unconstrained F1 column is not monotone because span/threshold are re-selected per variant (24/12/30/18).
\end{tablenotes}
\end{threeparttable}
\end{table}

Every semantic component contributes to the risk-state target: under the controlled comparison the full index is best on both F1 and AUC, and stance polarization---despite the smallest nominal weight ($w_s=0.2$)---is the most influential component, especially for AUC. This reconciles the feature-ablation finding of Table~\ref{tab:feature_ablation}: the classifier's validation selection prefers the state history alone because the index itself is the semantic carrier; removing a component at the index level, where its effect is re-expressed through the target definition, measurably degrades the task. The drop-attack variant is a partial exception in the unconstrained column---test F1 stays flat while AUC drops from $0.891$ to $0.839$---indicating that attack contributes predictable variance that a threshold-tuned classifier does not fully exploit.
...
\textbf{(c) Signal-to-noise ratio of the conflict index.} The conflict index pipeline introduces noise at four stages: BERT sentiment inference (a product-review model applied to argumentative text), three-component fusion, ECDF calibration, and top-$k$ aggregation. The measurement error of the first stage is quantified in Sec.~\ref{sec:construct_validity}: the attack score separates human-annotated attack comments at AUC $0.744$---above chance but far from ceiling---so the noise is bounded yet substantial. By contrast, comment volume requires only counting and log-normalization. The lag-1 autocorrelation of the raw conflict index averages only $0.112 \pm 0.140$ across Zhihu topics, confirming a near-absence of temporal structure---a fundamental requirement for any forecasting model.

\textbf{(d) Platform characteristics.} Zhihu is a moderated Q\&A platform where discussions are predominantly reasoned rather than confrontational. The Weibo reversal events, by contrast, are acute controversies with many emotionally charged comments per hour. Raw continuous regression on Weibo remains near zero (R\textsuperscript{2}=$0.018$), consistent with the boundary analysis above. The risk-state formulation, however, transfers successfully: the identical pipeline reaches test AUC $0.936$ on Weibo (Table~\ref{tab:weibo_results}), close to Zhihu's $0.891$, although the strongly autocorrelated reversal dynamics make persistence even stronger there (AUC $0.955$). Density and platform context therefore affect the \emph{relative} advantage over persistence, not the viability of the pipeline.
...
\textbf{(1) Construct validity of the conflict index.} The index relies on pretrained models not designed for conflict detection. We have validated the attack component against 191,634 human-annotated English Wikipedia Detox comments~\cite{Wulczyn2017}: the review-domain sentiment score separates attack-labeled comments at AUC $0.744$ (Spearman $\rho=0.254$), significantly above chance but far below dedicated toxicity classifiers, and a social-media-domain model reaches $0.811$ on the same labels. The emotion and stance components, and Chinese-language discourse, have not yet been validated by human raters; expert annotation is ongoing. The index must therefore be treated as a validated-but-moderate proxy rather than a fully validated measure of conflict.
...
\textbf{(5) Cross-platform and cross-lingual generalization.} The full pipeline is evaluated on two Chinese-language platforms (Zhihu, Weibo) and transfers across them (Zhihu AUC $0.891$; Weibo AUC $0.936$). English-language evidence currently covers only the attack component (Wikipedia Detox, AUC $0.744$); the emotion and stance components and the full pipeline on English discourse remain untested.
...
The immediate next step is not further architectural tuning but measurement validation. We have completed one half of that program: the attack component is validated against 191,634 human Wikipedia Detox labels (AUC $0.744$, Spearman $\rho=0.254$), and the pipeline transfers to Weibo reversal events (AUC $0.936$) with persistence dominating the relative comparison. The remaining half---expert annotation of the emotion and stance components and of Chinese-language discourse---is ongoing. Only a fully validated target should then be evaluated across platforms, event-held-out splits, aggregation scales, and longer horizons, with external precursor signals added where positive lead time is required.
