...
\begin{abstract}
Social media conflict forecasting promises earlier intervention than post-hoc content classification, yet raw text-derived scores are sparse and temporally noisy. We construct a conflict index without task-specific labels from three interpretable components---attack intensity from a multilingual toxicity detector, negative-arousal emotion from a zero-shot NLI anger classifier, and stance polarization from semantic camp clustering---and introduce a leakage-resistant cleaning pipeline that regularizes the 12-hour time axis, learns all calibration and cleaning statistics from training data only, and performs causal exponential smoothing. A logistic model predicts whether the stabilized state will exceed the topic-specific training-period 80th percentile within the next six bins. Across 20 Zhihu topics and 1,706 test windows, it achieves AUC $0.897$ and F1 $0.767$ against a persistence baseline of F1 $0.764$, with a significant recall gain ($+0.098$; 95\% CI $[0.042,0.167]$) and stable performance on entirely unseen topics (5-fold topic-held-out F1 $0.787\pm0.020$ vs.\ persistence $0.705$). The attack component is validated on 191,634 human Wikipedia Detox labels (AUC $0.988$, Spearman $\rho=0.507$); leave-one-out ablation identifies it as the most informative component. The same pipeline transfers to 20 Weibo reversal events (AUC $0.989$), and a reproduction of pooled random splits quantifies R\textsuperscript{2} inflation from protocol leakage on real data.
\end{abstract}
...
We operationalize a candidate \emph{conflict index} by combining attack-related negativity, negative high-arousal emotion, and stance polarization into a unified post-level score without task-specific conflict labels, using a genuine multilingual toxicity detector for attack intensity and a zero-shot NLI anger classifier for emotion. This unsupervised construction stage is distinct from the downstream forecasters, which are trained with historical trajectory targets. Its attack component is validated against 191,634 human-annotated English Wikipedia Detox comments (Spearman $\rho=0.507$, AUC $0.988$), the two stance camps are stable and balanced across topics (mean adjusted Rand index $0.95$), and leave-one-out ablation identifies the attack component as the most informative.
...
On held-out Zhihu periods, the selected classifier achieves AUC $0.897$ with a significant recall gain over persistence, and---unlike exact-value regression---generalizes to entirely unseen topics (5-fold topic-held-out F1 $0.787\pm0.020$ vs.\ persistence $0.705$). Five strong baselines (BiLSTM, GRU, TCN, Transformer, XGBoost) under the identical protocol fail to beat the simple logistic model, showing that signal quality, not architecture, determines performance. The pipeline also transfers to 20 Weibo reversal events (AUC $0.989$). Threshold and feature ablations show that causal state history drives the gain, while exact-value regression remains persistence-dominated.

\subsection{Paper Structure Overview}
The remainder of this paper is organized as follows. Section~II reviews related studies in sentiment analysis, harmful content detection, and temporal modeling. Section~III presents conflict-index construction and the two temporal prediction tasks, including the leakage-free stance camps and ECDF calibration. Section~IV reports the experiments: synthetic continuous forecasting, real-data risk-state prediction with signal cleaning, cross-platform transfer, measurement validation, ablation studies, boundary analysis, and limitations.
...
\end{equation}
where $\sigma(\cdot)$ is the sigmoid function, $w_a,w_e,w_s \ge 0$ control the relative contributions (e.g., $w_a:w_e:w_s = 0.5:0.3:0.2$), and $b$ is a bias term (we use $b=0$ in practice). The tildes denote \emph{distribution-calibrated} scores (Sec.~\ref{sec:calibration}) so that components from different sources become comparable.
...
\subsubsection{Stance Polarization via Training-Period Semantic Camps}
In the absence of stance labels (pro/con/neutral), we estimate polarization by measuring whether the comments of a topic form two semantically separated camps. To keep the procedure leakage-free, we fit $k{=}2$ clustering (k-means) on the sentence embeddings of the training-period comments only and keep the two centroids fixed when scoring validation and test comments, mirroring the training-only fitting of the ECDF calibrators (Sec.~\ref{sec:calibration}). This per-topic formulation---rather than within-bin clustering---is required because many 12-hour bins contain too few comments for stable within-bin clustering. We first compute sentence embeddings using a frozen encoder $\mathrm{Enc}(\cdot)$ (e.g., multilingual BERT pooling or sentence-transformer):
\begin{equation}
\mathbf{v}_{t,i} = \mathrm{Enc}(x_{t,i}) \in \mathbb{R}^d.
\label{eq:embedding}
\end{equation}
The two training-period centroids $\mathbf{u}_1$ and $\mathbf{u}_2$ define the topic-level inter-camp separation
\begin{equation}
\Delta = 1 - \cos(\mathbf{u}_{1}, \mathbf{u}_{2}) \in [0,2],
...
\begin{equation}
q_{t,i} = \frac{\left|d(\mathbf{v}_{t,i},\mathbf{u}_{1}) - d(\mathbf{v}_{t,i},\mathbf{u}_{2})\right|}
{d(\mathbf{v}_{t,i},\mathbf{u}_{1}) + d(\mathbf{v}_{t,i},\mathbf{u}_{2}) + \epsilon} \in [0,1],
...
For the real-data experiment, we implement the conflict-index pipeline as described in Algorithm~1. Attack intensity is obtained from a pretrained multilingual toxicity detector~\cite{hanu2020multilingualtoxicity} ($h_\psi$) that outputs the probability of toxic/attack-like content for a given text, and negative-arousal emotion scores are obtained from a zero-shot NLI classifier ($g_\omega$)~\cite{xnliModel} over the candidate labels ``negative'' and ``anger'', with $e = 0.7\,p_{\text{neg}} + 0.3\,p_{\text{anger}}$ as in (\ref{eq:emotion_score}). Sentence embeddings for stance polarization are obtained via a multilingual MiniLM-L12-v2 model~\cite{wang2020minilm} (118M parameters, 384-dimensional embeddings). For each topic, the stance K-means centers and all three ECDF quantile transformers are fitted only on comments before the exact 60\% regular-grid cutoff; the fixed models then score validation and test comments. The default fusion weights are $w_a:w_e:w_s = 0.5:0.3:0.2$; a Dirichlet-distributed search over 40 weight configurations (Sec.~\ref{sec:sensitivity}) shows test F1 within $[0.712,0.828]$ and identifies stance-dominant configurations with a secondary attack weight as the best. The top-$k$ aggregation fraction is $\rho=0.15$.
...
\subsubsection{Real-Data Risk-State Prediction (RQ2)}
...
\textbf{Zhihu cleaned risk-state prediction (positive result).} The regularized time axis yields 1,428 training, 680 validation, and 1,706 test windows (the shares 37\%/18\%/45\% differ from the 60/15/25 time split because early training bins are sparser and more windows are discarded by the future-observation filter, while the final quarter of each topic is the densest period). Validation selects EWMA span 48 and the full feature pool. On the untouched test segment, the classifier achieves AUC $0.897$, precision $0.803$, recall $0.735$, and F1 $0.767$ (Table~\ref{tab:real_results}). Persistence has higher AUC ($0.906$) and precision ($0.949$), but lower recall ($0.639$); the two methods tie on F1 ($0.764$ vs.\ $0.767$). Topic-level paired bootstrap estimates a recall gain of $+0.098$ (95\% CI $[0.042,0.167]$); the F1 difference is not significant ($+0.005$, CI $[-0.035,0.043]$). Recall improves on 15/20 topics. The stronger persistence baseline reflects the higher autocorrelation of the more accurate index: genuine attack/anger components make the risk state smoother and thus easier to extrapolate, which compresses the relative advantage of the learned classifier while raising absolute accuracy. Continuous six-step Ridge regression is positive (R\textsuperscript{2}=$0.465$) but remains below persistence ($0.640$); the supported claim is improved high-risk-state prediction, not superior exact-value forecasting.

\textbf{Weibo comment-volume forecasting (positive result).} To verify that the forecasting architecture itself is not the limiting factor, we apply the same LSTM forecaster to a structurally simpler but objectively measurable target: comment volume (log-transformed) on the 27-event Weibo public-opinion reversal dataset~\cite{Zhang2023Reversal} at 1-hour granularity. Unlike the conflict index---which introduces noise through text-model inference---comment volume is a direct, high-signal metric. Pooling all 27 events under temporal split yields R\textsuperscript{2}$=0.501$ (90,676 held-out windows; CNN-BiLSTM agrees at $0.485$), confirming that the LSTM architecture performs well on real-world time series when the target variable has genuine temporal autocorrelation. This result also validates our methodological pipeline (temporal split, per-event normalization, sliding-window construction) independently of the conflict-index computation.

\subsubsection{Classifier Complexity Does Not Help (RQ2)}

To test whether a stronger classifier could beat the simple logistic model on the risk-state task, we evaluate BiLSTM, GRU, TCN, Transformer, and XGBoost under the identical protocol---the same 60/15/25 temporal split, the same span-48 full-pool features, the same 1,706 test windows, and validation-based decision thresholds. Table~\ref{tab:risk_baselines} reports mean $\pm$ std over five seeds. No baseline improves on logistic: test AUC ranges from $0.853$ (TCN) to $0.887$ (GRU) versus $0.897$ for logistic, and test F1 from $0.736$ (TCN) to $0.763$ (XGBoost) versus $0.767$. Paired topic-bootstrap intervals for every model against persistence contain zero. With only 1,428 training windows drawn from 20 topics, deeper architectures plateau at the signal's ceiling; model complexity does not substitute for signal quality. This mirrors the synthetic-regression finding of Fig.~\ref{fig:model_comp} and supports the paper's focus on cleaning and calibration rather than architecture.

\begin{table}[H]
\caption{Strong baselines on the Zhihu risk-state task (same temporal protocol, same 1,706 test windows; mean $\pm$ std over 5 seeds).}\label{tab:risk_baselines}
\centering
\scriptsize
\begin{tabular}{lcccc}
\toprule
...
\midrule
Persistence & 0.906 & 0.949 & 0.639 & 0.764 \\
Logistic (ours) & \textbf{0.897} & \textbf{0.803} & 0.735 & \textbf{0.767} \\
BiLSTM & $0.878\pm0.003$ & 0.770 & 0.720 & $0.743\pm0.006$ \\
GRU & $0.887\pm0.004$ & 0.797 & 0.714 & $0.753\pm0.007$ \\
TCN & $0.853\pm0.024$ & 0.741 & 0.732 & $0.736\pm0.018$ \\
Transformer & $0.866\pm0.004$ & 0.750 & 0.774 & $0.761\pm0.010$ \\
XGBoost & $0.870\pm0.004$ & 0.789 & 0.743 & $0.763\pm0.005$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Weibo conflict-index forecasting (negative result).} To test whether the conflict index's failure on Zhihu is attributable to data sparsity alone, we compute the same BERT-based conflict index on the densest Weibo event (Zhang Zhehan incident, 22K comments, 62 comments/hour) at 1-hour bins. Despite a 90-fold increase in comment density relative to Zhihu (0.69 vs.\ 62 comments per hour on retained bins) and strong volume predictability (R\textsuperscript{2}$=0.501$), the conflict index still yields R\textsuperscript{2}$=0.018$---effectively zero. This result rules out sparsity as the sole explanation. The conflict index fails not because there are too few comments, but because text-level attack, emotion, and stance signals lack the temporal autocorrelation needed for short-horizon forecasting, regardless of data density.

Together, these experiments establish a hierarchy of forecastability: objective activity signals are strongly predictable; cleaned causal state histories support useful high-risk-state prediction; and exact raw semantic trajectories remain difficult even in dense commenting environments. The selected classifier uses the full feature pool, which combines the state history with activity and semantic components.

\begin{table}[H]
\caption{Zhihu high-risk-state prediction on the final 25\% test segment (true attack/anger models).}\label{tab:real_results}
\centering
\begin{threeparttable}
...
\textbf{Method} & \textbf{AUC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
Persistence & \textbf{0.906} & \textbf{0.949} & 0.639 & 0.764 \\
Logistic (state history) & 0.897 & 0.803 & \textbf{0.735} & \textbf{0.767} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item $N=1{,}706$ test windows from 20 topics. All model choices use validation data only. Paired topic-bootstrap 95\% CI: recall $+0.098$ $[0.042,0.167]$; F1 $+0.005$ $[-0.035,0.043]$; AUC $-0.010$ $[-0.020,-0.001]$. The stronger persistence baseline reflects the higher autocorrelation of the more accurate index.
\end{tablenotes}
\end{threeparttable}
\end{table}

\begin{table}[H]
\caption{Risk-threshold sensitivity on the Zhihu test segment (span 48, full feature pool).}\label{tab:threshold_sensitivity}
\centering
\begin{tabular}{lccccc}
\toprule
$Q^{\mathrm{train}}$ & \textbf{Prev.} & \textbf{AUC} & \textbf{Recall} & \textbf{F1} & \textbf{Pers.-F1} \\
\midrule
70th & 0.486 & 0.887 & 0.713 & 0.763 & 0.769 \\
80th & 0.448 & 0.897 & 0.735 & 0.767 & 0.764 \\
90th & 0.367 & 0.890 & 0.732 & 0.745 & 0.756 \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]
\caption{Feature ablation at the selected EWMA span (48).}\label{tab:feature_ablation}
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Features} & \textbf{Val.-AUC} & \textbf{Precision} & \textbf{Recall} & \textbf{Val.-F1} \\
\midrule
State only & 0.902 & 0.776 & 0.801 & 0.788 \\
State + activity & 0.896 & 0.727 & 0.829 & 0.775 \\
State + semantics & 0.901 & 0.766 & 0.815 & 0.790 \\
Full pool & \textbf{0.903} & \textbf{0.778} & \textbf{0.822} & \textbf{0.799} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Cross-Platform Generalization: Weibo Reversal Events}\label{sec:weibo_transfer}

To test whether the pipeline transfers to a different platform and discourse regime, we run the identical cleaning and risk-state protocol on the Weibo Public Opinion Reversal Dataset~\cite{Zhang2023Reversal} (27 events; our scrape contains 224,781 comments). Because the events last only 2--3 weeks, a 12-hour grid would leave at most 1--3 test windows per event; we therefore discretize at 2-hour resolution (the granularity of the original case study) and leave every other element of the pipeline---EWMA cleaning, training-period 80th-percentile threshold, validation-based span and feature selection, and the final-25\% test holdout---unchanged. Of the 27 events, 20 pass the minimum-length filter (at least 72 retained bins), yielding $N=275$ pooled test windows.

\begin{table}[H]
\caption{Weibo high-risk-state prediction on the held-out test segment (2-hour bins).}\label{tab:weibo_results}
\centering
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{AUC} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
Persistence & \textbf{0.990} & \textbf{0.971} & 0.957 & \textbf{0.964} \\
Logistic (state history) & 0.989 & 0.778 & \textbf{1.000} & 0.875 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item $N=275$ test windows from 20 events. Paired event-bootstrap 95\% CIs for the logistic-minus-persistence difference: recall $+0.053$ $[0.000,0.195]$; F1 $-0.105$ $[-0.285,0.010]$; AUC $-0.004$ $[-0.030,0.004]$.
\end{tablenotes}
\end{threeparttable}
\end{table}

The pipeline transfers cleanly and with higher absolute accuracy than on Zhihu: test AUC reaches $0.989$ (logistic) with perfect recall ($1.000$), and the AUC gap to persistence closes to $0.001$. On acute reversal events the persistence baseline remains extremely strong (AUC $0.990$, F1 $0.964$): the accurate index is so autocorrelated that carrying the last observation forward already performs well, and the logistic F1 stays below persistence ($0.875$ vs.\ $0.964$). The learned classifier nonetheless achieves essentially equal AUC with a recall gain of $+0.053$ (bootstrap CI $[0.000,0.195]$---the lower bound is exactly zero, so the recall advantage is marginal), at the cost of precision ($0.778$ vs.\ $0.971$). We therefore read the Weibo result as evidence of transferability with an honest boundary: the relative F1 advantage over persistence that motivated the Zhihu setup does not replicate on nearly persistent reversal-event series.

\subsection{Generalization to Unseen Topics and Events}

The time-held-out evaluation above pools windows from all topics during classifier training, so it does not test whether the model transfers to a topic it has never seen. We therefore re-run the identical classifier protocol (regularization and decision threshold selected on a disjoint validation set of topics) under 5-fold topic-held-out rotation on Zhihu and 5-fold event-held-out rotation on Weibo, always training on windows from a subset of topics/events and testing exclusively on windows from held-out topics/events.

On Zhihu, the held-out topics achieve test F1 $0.787\pm0.020$ (mean $\pm$ std across folds) against persistence $0.705$, AUC $0.847$; the paired topic-bootstrap F1 difference is $+0.086$, larger than under time-held-out evaluation, because persistence is weaker on unseen topics. On Weibo, event-held-out F1 is $0.744\pm0.142$ against persistence $0.738$ (AUC $0.912$); the advantage is small and high-variance, consistent with the near-persistent Weibo dynamics. The classifier therefore generalizes to entirely unseen topics on Zhihu, and the transfer result on Weibo is not an artifact of training on the same events.

\subsection{Construct Validity of the Attack Component}\label{sec:construct_validity}

The conflict index is unsupervised by construction, so its measurement quality must be checked against human judgments. We validate the attack component---the largest-weight element of the fusion---on the Wikipedia Detox personal-attacks corpus~\cite{Wulczyn2017}: 191,634 English comments with binary human labels (10.1\% positive). Using the pipeline's exact scoring path (the multilingual toxicity model, 256-token truncation), the toxicity score separates attack-labeled from unlabeled comments at AUC $0.988$ (Spearman $\rho=0.507$, Pearson $r=0.852$; both $p<0.001$), and the mean score is $0.806$ for labeled versus $0.033$ for unlabeled comments---a separation nearly two orders of magnitude larger than the review-sentiment proxy used in an earlier version of this pipeline (AUC $0.744$, $\rho=0.254$). The attack component is therefore a statistically strong measure of personal attack. The anger component (zero-shot NLI) and the stance component are evaluated structurally (Sec.~\ref{sec:stance_audit}) rather than against external labels; expert annotation of the Chinese-language components remains ongoing work.

\subsection{Stance-Cluster Audit: Two Stances or Two Subtopics?}\label{sec:stance_audit}

The stance component assumes that the two $k{=}2$ camps correspond to pro/con positions rather than to unrelated subtopics. We audit this assumption on all 20 Zhihu topics by reproducing the pipeline's training-period KMeans and examining (i) cluster balance, (ii) clustering stability across random seeds (adjusted Rand index, ARI), (iii) stance-lexicon hit rates (pro-words such as ``支持/赞同/力挺'' vs.\ anti-words such as ``反对/抵制/谴责''), and (iv) representative comments per cluster.

Across topics, the two clusters are moderately balanced (mean size ratio $0.757$, minimum $0.32$) and highly stable: the mean ARI across KMeans seeds is $0.948$ (minimum $0.787$), so the two-camp structure is not an artifact of initialization. Stance-lexicon hit rates differ between clusters in the expected direction on most topics (mean absolute pro-lexicon difference $0.062$, anti-lexicon difference $0.037$; per-topic maxima $0.145$ and $0.087$), and the top TF-IDF terms of the two clusters are dominated by stance-related and topic-entity words rather than disjoint factual subtopics. Representative comments confirm that cluster 0 typically expresses the affirmative side of the topic question and cluster 1 the critical side. The audit therefore supports the two-stance reading for the majority of topics, while the smaller-balance topics (ratio $\approx 0.3$) are cases where one camp dominates the discourse---a meaningful property of polarized debates rather than a clustering failure.

\subsection{Ablation Studies}
\label{sec:ablation}

\subsubsection{Component Contribution and Weight Sensitivity (RQ4)}\label{sec:component_ablation}

To assess how input-signal quality affects downstream prediction, we conduct two complementary analyses. First, we inject Gaussian noise of increasing magnitude ($\sigma \in \{0.03, 0.06, 0.10\}$) into the synthetic trajectories. Figure~\ref{fig:ablation} and Table~\ref{tab:ablation} show the expected degradation as deliberately embedded temporal structure is corrupted. Second, we perform a leave-one-out ablation at the \emph{index-construction} level on the risk-state task: for each variant, the comment-level index is recomputed from the remaining components (weights renormalized to unit sum), trajectories are rebuilt, and the full cleaning and classification pipeline is re-executed, with the test segment evaluated only once. Table~\ref{tab:component_ablation} reports both the pipeline-selected configuration and a controlled comparison at the fixed span (24) with the full feature pool. The fixed comparison uses span 24---the midpoint of the candidate grid---rather than the selected span 48, because the longer smoothing scale absorbs most component-level variation and would mask the components' marginal contributions; the unconstrained columns report each variant's own selected span.

\begin{table}[H]
\caption{Leave-one-out component ablation on the Zhihu risk-state task (test set). The $^{*}$ columns evaluate every variant at span 24 with the full feature pool.}\label{tab:component_ablation}
\centering
\footnotesize
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
\textbf{Index} & $(w_a,w_e,w_s)$ & \textbf{F1} & \textbf{AUC} & \textbf{F1$^{*}$} & \textbf{AUC$^{*}$} \\
\midrule
Full & 0.50, 0.30, 0.20 & 0.767 & \textbf{0.897} & 0.804 & 0.860 \\
Drop attack (A) & 0.00, 0.60, 0.40 & 0.771 & 0.830 & 0.740 & \textbf{0.862} \\
Drop emotion (E) & 0.71, 0.00, 0.29 & 0.800 & 0.833 & 0.806 & 0.856 \\
Drop stance (S) & 0.63, 0.38, 0.00 & \textbf{0.822} & 0.824 & \textbf{0.821} & 0.867 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item Fixed-span deltas vs.\ Full: dropping A $\Delta$F1 $=-0.064$, $\Delta$AUC $=+0.002$; dropping E $\Delta$F1 $=+0.002$, $\Delta$AUC $=-0.004$; dropping S $\Delta$F1 $=+0.017$, $\Delta$AUC $=+0.007$. The attack component---the strongest single signal of the new model---is the only component whose removal degrades F1; emotion and stance contribute little at the index level, and the unconstrained columns are not monotone because span/threshold are re-selected per variant.
\end{tablenotes}
\end{threeparttable}
\end{table}

With the true attack/anger models, the component ranking changes: under the controlled fixed-span comparison, the attack component is the only one whose removal degrades test F1 ($\Delta$F1 $=-0.064$), confirming that the validated toxicity signal (Sec.~\ref{sec:construct_validity}) carries most of the predictive information. Emotion contributes nothing at the index level ($\Delta$F1 $=+0.002$), and removing stance even slightly improves F1 ($+0.017$)---the stable two-camp structure of Sec.~\ref{sec:stance_audit} is largely orthogonal to the risk-state target. This complements the feature ablation of Table~\ref{tab:feature_ablation}, where the full feature pool is selected: with stronger components, the classifier exploits the raw semantic features directly rather than relying on the state history alone.

\subsubsection{Fusion Weight Sensitivity Analysis}\label{sec:sensitivity}

To assess sensitivity to fusion weights $(w_a, w_e, w_s)$, we re-run the risk-state pipeline for 40 configurations sampled from a Dirichlet distribution (fixed span 24, full feature pool). Test F1 ranges over $[0.712, 0.828]$ (mean $0.784$, std $0.027$), and the best configuration gives stance the largest weight, $(w_a,w_e,w_s) = (0.26, 0.00, 0.74)$, with F1 $0.828$ and AUC $0.919$. The default $(0.5, 0.3, 0.2)$ lies near the middle of this range. Consistent with the component ablation, configurations that down-weight the attack component lose F1, while the emotion weight contributes little; a large stance weight can partially compensate for attack in the fusion, but not when attack is removed entirely.
...
\caption{Component ablation on the risk-state task (test set): fixed-span F1 and AUC for the full index and each leave-one-out variant.}\label{fig:comp_ablation}
\end{figure}
...
\subsubsection{Random-Split Inflation: A Reproduction of the IPSO-LSTM Evaluation Protocol}\label{sec:random_split}

LSTM-variant forecasting studies of the IPSO-LSTM line~\cite{Mu2023IPSO} typically pool windows across series and split them randomly without temporal blocking. Because successive windows overlap and windows from the same series appear in both partitions, this protocol can leak the recent past into the ``test'' set. We reproduce this evaluation protocol---pooled windows with a 70/30 random split---against per-series temporal splitting on the real Zhihu trajectories and on independent synthetic trajectories, using identical models (LSTM, CNN-BiLSTM, XGBoost), identical windows ($L{=}12$, $H{=}6$), and per-series z-scoring. We reproduce the protocol rather than the PSO hyperparameter search of~\cite{Mu2023IPSO}.

\begin{table}[H]
\caption{Random versus temporal splitting on real Zhihu and synthetic trajectories (R\textsuperscript{2} on pooled test windows).}\label{tab:split_protocol}
\centering
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{\textbf{Random split}} & \multicolumn{2}{c}{\textbf{Temporal split}} \\
\textbf{Model} & Real Zhihu & Synthetic & Real Zhihu & Synthetic \\
\midrule
LSTM & $-0.030$ & $0.697$ & $-0.074$ & $0.769$ \\
CNN-BiLSTM & $0.066$ & $0.774$ & $-0.078$ & $0.843$ \\
XGBoost & $-0.110$ & $0.765$ & $-0.126$ & $0.733$ \\
\bottomrule
\end{tabular}
\end{table}

On real data the random protocol inflates R\textsuperscript{2} for every model (CNN-BiLSTM $+0.144$; LSTM $+0.043$; XGBoost $+0.015$), and for CNN-BiLSTM the escalation-detection F1 jumps from $0.000$ under the temporal protocol to $0.516$ under the random protocol. On independent synthetic series no inflation appears (LSTM $-0.072$; CNN-BiLSTM $-0.070$; XGBoost $+0.032$), because shuffling cannot leak across independent draws---temporal splitting even helps there by preserving more training data per series. The inflation is therefore driven by real cross-topic sharing of comment-stream dynamics, and it quantifies why optimistic literature results can arise from protocol choice alone.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/fig_case_study.pdf}
\caption{Conflict index trajectories for the three Weibo reversal events. The shaded regions mark the annotated reversal windows.}\label{fig:case_study}
\end{figure}

\subsection{Case Study: Real-World Public Opinion Reversal Events}

To validate the framework on real-world conflict escalation data, we conduct a case study on the \textbf{Weibo Public Opinion Reversal Dataset}~\cite{Zhang2023Reversal} (27 events; our scrape contains 224,781 comments). This case study targets the \emph{activity} signal and the annotated reversal windows at 2-hour granularity; it complements the risk-state transfer experiment of Sec.~\ref{sec:weibo_transfer}, which evaluates the cleaned semantic index on 20 events at 2-hour resolution. The three densest events---Huolala passenger death (32K comments, 335h span), Zhang Zhehan incident (22K comments, 359h), and Gou Jing exam fraud (20K comments, 373h)---are selected for analysis at 2-hour bin granularity. The reversal window annotation provides ground-truth periods of elevated controversy. As reported in Sec.~V, comment volume on these dense events is strongly predictable (pooled R\textsuperscript{2}=0.501), while the raw text-derived conflict index yields near-zero R\textsuperscript{2}. Figure~\ref{fig:case_study} visualizes the conflict index trajectories for all three events. The red shaded regions mark the annotated reversal windows---periods during which public opinion consensus underwent a dramatic shift according to the dataset creators~\cite{Zhang2023Reversal}. Across the three events, the conflict index rises modestly within the reversal window (Huolala $\Delta c=+0.011$, Zhang Zhehan $+0.018$, Gou Jing $+0.008$), indicating a weak cross-sectional association. However, CNN-BiLSTM escalation detection is unreliable on these short trajectories (Esc-F1: Huolala 0.89, Zhang Zhehan 0.24, Gou Jing 0.00), with the lone high score on Huolala driven by the event's unusually concentrated conflict dynamics. The reversal window annotation measures \emph{opinion change}, not \emph{comment-level conflict intensity}, and the two constructs are not equivalent.
...
\textbf{(a) Comment sparsity.} The retained Zhihu bins average 8.26 records (22,399 records across 2,713 retained bins), but many unretained 12-hour intervals are empty or contain fewer than three records. With the top-$k$ aggregator ($\rho{=}0.15$), only one or two comments determine $\bar{c}_t$ in many retained bins, making the aggregate a noisy small-sample estimator.
...
\textbf{(c) Signal-to-noise ratio of the conflict index.} The conflict index pipeline introduces noise at four stages: toxicity-model inference, zero-shot NLI emotion inference, ECDF calibration, and top-$k$ aggregation. The measurement error of the first stage is quantified in Sec.~\ref{sec:construct_validity}: the attack score separates human-annotated attack comments at AUC $0.988$---near ceiling on the English Detox benchmark---so the attack signal is strong; the remaining stages (especially the top-$k$ aggregation on sparse bins) still limit the raw trajectory's temporal structure. By contrast, comment volume requires only counting and log-normalization. The lag-1 autocorrelation of the raw conflict index averages only $0.112 \pm 0.140$ across Zhihu topics, confirming a near-absence of temporal structure---a fundamental requirement for any forecasting model.

\textbf{(d) Platform characteristics.} Zhihu is a moderated Q\&A platform where discussions are predominantly reasoned rather than confrontational. The Weibo reversal events, by contrast, are acute controversies with many emotionally charged comments per hour. Raw continuous regression on Weibo remains near zero (R\textsuperscript{2}=$0.018$), consistent with the boundary analysis above. The risk-state formulation, however, transfers successfully: the identical pipeline reaches test AUC $0.989$ on Weibo (Table~\ref{tab:weibo_results}), above Zhihu's $0.897$, although the strongly autocorrelated reversal dynamics make persistence equally strong there (AUC $0.990$). Density and platform context therefore affect the \emph{relative} advantage over persistence, not the viability of the pipeline.

The analysis points to (c) as the dominant difficulty, amplified by (a). The positive classifier succeeds by changing the operational question from exact score extrapolation to whether a persistent, topic-relative high-risk state will occur. Validation selects the full feature pool: state history alone already carries most of the signal, and the activity and semantic features add a small validation gain (Table~\ref{tab:feature_ablation}). This target is appropriate for monitoring, but it should not be confused with precise prediction of every raw conflict-score fluctuation.

\subsection{Limitations}\label{sec:limitations}

\textbf{(1) Construct validity of the conflict index.} The index relies on pretrained models not designed for conflict detection. We have validated the attack component against 191,634 human-annotated English Wikipedia Detox comments~\cite{Wulczyn2017}: the multilingual toxicity detector separates attack-labeled comments at AUC $0.988$ (Spearman $\rho=0.507$), a strong result on English data. The anger component (zero-shot NLI) and the stance clusters have been validated only structurally (Sec.~\ref{sec:stance_audit}), and Chinese-language human ratings are still missing; expert annotation is ongoing. The index must therefore be treated as a strong proxy for attack intensity with partially validated emotion and stance components.
...
\textbf{(4) Monitoring horizon.} The classifier predicts threshold exceedance within six 12-hour bins, whereas the selected 24-day smoothing scale represents a slower-moving risk state. This supports medium-scale monitoring rather than rapid event-onset warning. The negative synthetic lead time ($-1.7$ bins) further limits claims of operational advance notice.

\textbf{(5) Cross-platform and cross-lingual generalization.} The full pipeline is evaluated on two Chinese-language platforms (Zhihu, Weibo) and transfers across them (Zhihu AUC $0.897$; Weibo AUC $0.989$). English-language evidence covers only the attack component (Wikipedia Detox, AUC $0.988$); the anger component, the stance component, and the full pipeline on English discourse remain untested.
...
After restoring the regular time grid, applying training-only calibration and robust cleaning, and selecting a causal smoothing scale and feature set on validation data, the risk-state classifier reaches AUC $0.897$ and F1 $0.767$ on 1,706 held-out Zhihu windows, with a significant recall gain over persistence ($+0.098$; 95\% CI $[0.042,0.167]$) and strong generalization to unseen topics (topic-held-out F1 $0.787\pm0.020$ vs.\ persistence $0.705$). Five strong baselines under the identical protocol do not beat the simple logistic model, confirming that signal quality, not architecture, drives performance. Continuous regression remains below persistence despite positive absolute R\textsuperscript{2}. The main finding is therefore both positive and bounded: with genuine attack and anger models, cleaned causal states support high-accuracy warning-state classification and cross-platform transfer (Weibo AUC $0.989$), but not superior exact trajectory prediction, and the relative F1 advantage over persistence shrinks as the index becomes more accurate and more persistent.

The immediate next step is not further architectural tuning but measurement validation. We have completed most of that program: the attack component is validated against 191,634 human Wikipedia Detox labels (AUC $0.988$, Spearman $\rho=0.507$), the stance clusters pass the stability and lexicon audit (Sec.~\ref{sec:stance_audit}), and the pipeline transfers to Weibo reversal events (AUC $0.989$) with persistence dominating the relative comparison. The remaining half---expert annotation of the anger component and of Chinese-language discourse---is ongoing. Only a fully validated target should then be evaluated across aggregation scales and longer horizons, with external precursor signals added where positive lead time is required.

\bibliographystyle{IEEEtran}
\bibliography{refs}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/photo_zhu.png}}]{Linli Zhu} is currently pursuing the B.S. degree in computer science at Ningxia University, Yinchuan, China. Her research interests include natural language processing, public opinion analysis, and computational social science. She was a recipient of the National First Prize in the National College Student Artificial Intelligence Security Competition.
\end{IEEEbiography}
