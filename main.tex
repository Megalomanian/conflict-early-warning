...
On the Huolala event, the conflict index increases by $+0.011$ during the reversal window (pre-reversal: 0.605 $\rightarrow$ reversal: 0.616). CNN-BiLSTM achieves R\textsuperscript{2}~=~0.081 versus $-$0.398 for persistence and detects escalations with F1~=~\textbf{0.889}. Across all three events, the BERT-based conflict index consistently captures escalation dynamics: the Zhang Zhehan incident shows R\textsuperscript{2}~=~0.254 and Esc-F1~=~0.240 (up from 0.000 with a keyword baseline), and the Gou Jing case achieves R\textsuperscript{2}~=~0.305 though with Esc-F1~=~0.000 due to low escalation prevalence. All three events exhibit positive conflict-index shifts during reversal windows under the BERT pipeline, confirming the framework's ability to capture authentic escalation dynamics on real-world data.
