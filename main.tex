...
\begin{abstract}
Social media discussions can escalate rapidly, making early identification of risky public-opinion dynamics important for timely intervention. Existing studies often focus on classifying sentiment or harmfulness at the level of individual posts, but they provide limited support for forecasting how topic-level conflict evolves over time. In this paper, we propose an unsupervised framework for risk forecasting of conflict escalation in social-media comment streams. The framework constructs a comment-level conflict index by combining interpretable signals of antagonistic intensity, negative high-arousal emotion, and stance polarization, without requiring manual annotation. These scores are aggregated into bin-level risk trajectories and modeled with a CNN-BiLSTM forecaster for short-horizon prediction. We evaluate the framework under a strict temporal-split protocol across three complementary scenarios. On synthetic conflict trajectories, CNN-BiLSTM achieves R\textsuperscript{2}=$0.809\pm0.010$, confirming that the architecture performs well under ideal conditions. On real Weibo reversal events with dense commenting activity, a simpler volume-based target yields R\textsuperscript{2}=$0.715$, validating the forecaster on real-world data. However, on the conflict index itself computed from sparse, topic-level Zhihu discussions, forecasting performance collapses (R\textsuperscript{2}{\approx}0), revealing that text-based conflict signals at coarse temporal granularity lack short-term predictability. This negative result is informative: it demonstrates that the conflict index is sensitive enough to expose the fundamental randomness of antagonistic comment arrival in low-density settings, and it highlights a critical gap between synthetic benchmarks and real-world deployability. We argue that honest reporting of both positive and negative results is essential for advancing reliable public-opinion risk monitoring.
\end{abstract}
...
We aggregate comment-level conflict scores into bin-level topic risk trajectories and use deep sequential forecasters (CNN-BiLSTM, BiGRU, Transformer) to model persistence, volatility, and rising trends. Through a three-scenario evaluation---synthetic trajectories, dense Weibo events, and sparse Zhihu discussions---we characterize when and why conflict can (and cannot) be forecast from short-horizon text signals, and we quantify the gap between benchmark performance and real-world deployability.
...
These two experiments establish a crucial boundary condition for our framework: conflict escalation can be forecast when commenting activity is dense and the target signal is objectively measurable, but not when the target is a text-derived index computed from sparse discussions. We return to the implications of this finding in Section~\ref{sec:discussion}.
...
\end{tabular}
\vspace{4pt}
\footnotesize{All results under strict temporal ordering with train-only normalization. Weibo data: 6,407 windows from 27 reversal events. Zhihu data: 2,373 windows from 20 topics.}
\end{table}
...
\subsection{Why Does the Conflict Index Fail on Sparse Data?}\label{sec:attribution}
...
\textbf{(1) Construct validity of the conflict index.} The conflict index has not been validated against human annotations of perceived conflict intensity. We recommend that future work conduct a validation study in which 2--3 annotators independently rate 200--500 comments on a conflict-intensity scale, with inter-annotator agreement measured via Krippendorff's $\alpha$, and the index evaluated via Spearman's $\rho$ against the aggregated human scores. This would establish the index as a validated measurement instrument rather than a heuristic.
