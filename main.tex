...
\subsection{Cross-Platform Generalization: Weibo Reversal Events (RQ2)}
...
\subsection{Construct Validity of the Attack Component}
...
\subsubsection{Random-Split Inflation: A Reproduction of Optimistic Protocols}

Published public-opinion forecasting papers frequently evaluate pooled windows under random $k$-fold splits without temporal blocking. Because successive windows overlap and windows from the same series appear in both partitions, this protocol can leak the recent past into the ``test'' set. We reproduce both protocols on the real Zhihu trajectories and on independent synthetic trajectories with identical models (LSTM, CNN-BiLSTM, XGBoost), identical windows ($L{=}12$, $H{=}6$), per-series z-scoring, and a 70/30 split.
