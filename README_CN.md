...
| 场景 | 预测目标 | 结果 | 含义 |
|------|---------|------|------|
| 合成冲突轨迹 | 连续冲突指数 | R² **0.809 ± 0.010** | 理想条件下架构有效 |
| 微博反转事件（27个池化） | 评论量 | R² **0.715** | 预测器在真实密集事件上可泛化 |
| 知乎讨论（20个话题） | 原始冲突指数回归 | R² **≈ 0** | 精确文本轨迹仍不可预测 |
| 知乎讨论（20个话题） | 清洗后的高风险状态 | F1 **0.778**、召回率 **0.822** | 因果状态历史优于Persistence（F1 0.716） |

**核心观点**：粗粒度下的精确文本冲突轨迹缺乏短期可预测性，但无泄露的时间轴修复和因果平滑可以支持更窄的高风险状态分类任务。该正向结果有明确边界：AUC 0.891，略低于Persistence的0.896。
...
| 数据集 | 预测目标 | 指标 | 结果 |
|--------|---------|------|------|
| 微博27事件，1h bins | 评论量（池化） | R² | **0.715** |
| 知乎20话题，12h bins | 原始冲突指数 | R² | ≈ 0 |
| 知乎20话题，最终25% | 清洗后的高风险状态 | Logistic F1 / 召回率 | **0.778 / 0.822** |
| 知乎20话题，最终25% | 清洗后的高风险状态 | Persistence F1 / 召回率 | 0.716 / 0.584 |
